...
-- Categories Queries
	...
	-- Query to get categories for drop down
...
-- For example, this gets all live courses, but on the business logic we could append, " AND CATEGORY ='someValue'"
SELECT DISTINCT courseId, courseName FROM Courses LEFT OUTER JOIN Categories ON categoryFk = categoryId
INNER JOIN LanguagesCourses ON courseId = courseFk INNER JOIN Languages ON languageFk = languageId WHERE isLive=1 AND 5=5
